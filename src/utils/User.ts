...
export class UserClass {
    ...
    static async createUser(createUserInput: CreateUserInput): Promise<User>  {
        ...
        const emailAlreadyExists = await this.fetchUserByEmail(createUserInput.email);
        if(emailAlreadyExists){
            throw new BadRequestError('This email is already taken. Please try another one.')
        }
    ...
    static async loginUser(loginUserInput: LoginUserInput): Promise<User> {
        ...
        const isMatch = Password.comparePasswords(loginUserInput.password, user.password)
        if(!isMatch){
            throw new BadRequestError('Bad credentials. Please try again');
        }
        return user;
    ...
    static async sendVerificationEmail(user: User){
        try {
            ...
            throw new TRPCError({code: 'INTERNAL_SERVER_ERROR', message: 'Email was not sent.'})
        }
        return true;
    }
}